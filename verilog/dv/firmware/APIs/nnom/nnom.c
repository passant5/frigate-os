...
nnom_layer_t *model_compile_Conv_Accelerator(nnom_layer_t *layer){
    if(layer->in->tensor->bitwidth == 8) {
        uint8_t relu_en = 0;
        uint8_t compatable_activation = 1;
        q7_t relu_max = 127;
        q7_t relu_threshold = 0;
        q7_t relu_negative_slope = 0;
        uint8_t pool_en = 0;
        nnom_conv2d_layer_t *current_layer = (nnom_conv2d_layer_t*)layer;
        nnom_conv2d_Accelerator_layer_t *accel_layer;
        size_t mem_size = sizeof(nnom_conv2d_Accelerator_layer_t) + sizeof(nnom_layer_io_t) * 2 + sizeof(nnom_buf_t);
        accel_layer = nnom_mem(mem_size);
        
        if(layer->actail!=NULL) {
            // ReLU
            if (layer->actail->type == ACT_RELU){ //todo:check this condition
                relu_en = 1;
                layer->actail = NULL;
            }else if (layer->actail->type == ACT_LEAKY_RELU){
                nnom_activation_leaky_relu_t* act = layer->actail;
                relu_en = 1;
                relu_negative_slope = act->alpha;
                layer->actail = NULL;
            }else if (layer->actail->type == ACT_ADV_RELU){
                nnom_activation_adv_relu_t* act = layer->actail;
                relu_en = 1;
                relu_max = act->max;
                relu_threshold = act->threshold;   // todo:  checkout how to compute
                relu_negative_slope = act->negative_slope;
                layer->actail = NULL;
            }else{
                compatable_activation=0;
            }
        }else{ }
        if (compatable_activation){
            #ifdef EFABLESS_ACCELERATOR_MAXPOOL

            if (layer->shortcut != NULL){
                if (layer->shortcut->type == NNOM_MAXPOOL){
                    nnom_maxpool_layer_t *ml = (nnom_maxpool_layer_t*)layer->shortcut;
                    pool_en=1;
                    accel_layer->pool_kernel         = ml->kernel;
                    accel_layer->pool_stride         = ml->stride;
                    accel_layer->pool_pad            = ml->pad;
                    accel_layer->pool_padding_type   = ml->padding_type;
                    accel_layer->pool_output_shift   = ml->output_shift;
        ...
        accel_layer->super           = current_layer->super;
        accel_layer->kernel          = current_layer->kernel;
        accel_layer->stride          = current_layer->stride;
        accel_layer->pad             = current_layer->pad;
        accel_layer->dilation        = current_layer->dilation;
        accel_layer->padding_type    = current_layer->padding_type;
        accel_layer->filter_mult     = current_layer->filter_mult;
        accel_layer->weight          = current_layer->weight;
        accel_layer->bias            = current_layer->bias;
        accel_layer->output_rshift   = current_layer->output_rshift;
        accel_layer->bias_lshift     = current_layer->bias_lshift;

        accel_layer->relu_en             = relu_en;
        accel_layer->relu_max            = relu_max;
        accel_layer->relu_threshold      = relu_threshold;
        accel_layer->relu_negative_slope = relu_negative_slope;

        accel_layer->mid_out = *layer->out;
        accel_layer->pool_en = pool_en;
        layer = (nnom_layer_t*) accel_layer;
        layer->run = conv2d_accelerator_run;
        if (pool_en){
            layer->out->tensor= layer->shortcut->out->tensor;
            layer->out->mem= layer->shortcut->out->mem;
            layer->shortcut = layer->shortcut->shortcut;
        }
    }
    return layer;
}
...
nnom_status_t model_compile(nnom_model_t *m, nnom_layer_t *input, nnom_layer_t *output)
{
    ...
    nnom_layer_t *layer = m->head;
    nnom_layer_t *prev = NULL;

#ifdef NNOM_USING_EFABLESS_ACCELERATOR
    while (layer) {
        if (layer->type == NNOM_CONV_2D) {
            layer = model_compile_Conv_Accelerator(layer);

            // If layer is the head, update the head pointer in m
            if (prev == NULL) m->head = layer;
            else prev->shortcut = layer;
        }
...
nnom_status_t layer_run(nnom_layer_t *layer)
{
	...
	if (layer->actail != NULL)
	{
        if (layer->actail->type != ACT_UNKNOWN)
...
nnom_status_t model_run_to(nnom_model_t *m, nnom_layer_t *end_layer)
{
	...
	{
		// if (toggle){
		// 	EF_GPIO8_writeData(PORT_C, 0x4); 
		// 	toggle = 0;
		// }
		// else {
		// 	EF_GPIO8_writeData(PORT_C, 0x5); 
		// 	toggle = 1;
		// }
		// run layer
