	...
	q7_t *Im_out)
{
    int16_t i_ch_in, i_x, i_y;
    int16_t k_x, k_y;
//    int relu_size = dim_im_in_x * dim_im_in_y*ch_im_in;
//    printf("\n\n");
...
/**
 * @brief Multiplies four q7_t components.
 * 
 * @param image_component Array of four q7_t components representing the image component.
 * @param weight_component Array of four q7_t components representing the weight component.
 * @return The sum of the element-wise multiplication of the image and weight components.
 */
long multiply_four_q7_components(q7_t image_component[4], q7_t weight_component[4]){
    /* Initialize the result to zero */
    long res =0;
    
    /* Multiply each element of the image and weight components, and add the results */
    for (int i = 0; i < 4; i++)
    {
        res += image_component[i] * weight_component[i];
    }
...
//                                    //printf("%d ", Image_channels[k][i*dim_im_in_x + j]);
//            }
//                            //printf("**");
//        }
//                //printf("\n");
...
//            one_channel_conv(   Image_channels[j],
//                             result,
//                             Kernel_filters[i][j],
//                             temp_bias,                                 // bias
//                             dim_im_in_x,                // input image dimention x
//                             dim_im_in_y,                // input image dimention y
//                             dim_kernel_x,               // filter kernel size x
//                             dim_kernel_y,               // filter kernel size y
//                             stride_x,                   // stride x
//                             stride_y,                   // stride y
//                             dim_im_out_x,               // output image dimension x
//                             dim_im_out_y,               // output image dimension y
...
//                for (k = 0; k < dim_im_out_y; k++)
//                    for (l = 0; l < dim_im_out_x; l++){
//                        int temp=resultchannels[i + (k * dim_im_out_x + l) * ch_im_out] ;
//                        Im_out[i + (k * dim_im_out_x + l) * ch_im_out]= (q7_t)__NNOM_SSAT(((temp + temp_bias) >> outshift), 8);
//                    }
...
//                        // ---------------------------------------------------------------------------------------
//                        // this is a testing section for 4 MAC operations acceleration
//                        if (ch_im_in % 4 == 0){ // channels are a multiple of 4
//                            for (l = 0; l < ch_im_in; l+=4){
//                                //q7_t image_buffer[4]= { Im_in[in_pix_loc + l],
//                                //                        Im_in[in_pix_loc + l+1],
//                                //                        Im_in[in_pix_loc + l+2],
//                                //                        Im_in[in_pix_loc + l+3]};

//                                //q7_t weight_buffer[4]= {    wt[wt_loc + l],
//                                //                            wt[wt_loc + l+1],
//                                //                            wt[wt_loc + l+2],
//                                //                            wt[wt_loc + l+3]};
                                                            
//                                //conv_out += multiply_four_q7_components(image_buffer,weight_buffer);
//                                conv_out +=     (Im_in[in_pix_loc + l] * wt[wt_loc + l]) 
//                                            +   (Im_in[in_pix_loc + l+1] * wt[wt_loc + l+1])
//                                            +   (Im_in[in_pix_loc + l+2] * wt[wt_loc + l+2]) 
//                                            +   (Im_in[in_pix_loc + l+3] * wt[wt_loc + l+3]);
//                            }
//                        } else {
//                            for (l = 0; l < ch_im_in; l++)
//                            {     
...
//                    }
//                }
//                    // saturates the output to q7 format
...
void local_convolve_CHW_q7_nonsquare(
    const q7_t *input_image,                 // input image
	const uint16_t dim_im_in_x,              // input image dimention x
	const uint16_t dim_im_in_y,              // input image dimention y
	const uint16_t ch_im_in,                 // number of input image channels
	const q7_t *wt,                          // kernel weights
    ...
    const nnom_qformat_param_t *out_shift,   // output shift
    const nnom_qtype_t q_type,               // per channel or per tensor
    q7_t *output_image,                      // output image
	const uint16_t dim_im_out_x,             // output image dimension x
	const uint16_t dim_im_out_y,             // output image dimension y
	q15_t *bufferA,                          //buffer space for input
	q7_t *bufferB                            //buffer space for output
				...
				for (m = 0; m < dim_kernel_y; m++)
				{
					for (n = 0; n < dim_kernel_x; n++)
					{
						...
						if (in_row >= 0 && in_col >= 0 && in_row < dim_im_in_y && in_col < dim_im_in_x)
						{
							...
							for (l = 0; l < ch_im_in; l++)
							{
                                int image_component = input_image[(in_row * dim_im_in_x + in_col) + l * dim_im_in_x * dim_im_in_y];
                                int weight_component = wt[(m * dim_kernel_x + n) * ch_im_in * ch_im_out + l * ch_im_out + i];
                                
								conv_out += image_component * weight_component;
							}
						}
					}
				}
                output_image[i * dim_im_out_x * dim_im_out_y + (j * dim_im_out_x + k)] = (q7_t)__NNOM_SSAT((conv_out >> out_shift[shift_idx]), 8);
            }
        }
    }
}
