...
def PROG_WR_BIN():
    file=open("gpio_test.bin","rb")
    Addr = 0
    while True:
        page=list(file.read(128))
        if (not page):
            break
        #print(Addr)
        #print(page)
        FLASH_WEN()
        FLASH_PAGE_PROG(Addr, page)
        time.sleep(0.1)
        Addr = Addr + 128
    file.close()
def PROG_RD():
...
#FLASH_BLOCK_ERASE(0)
#time.sleep(10)
#arr = [0x6F , 0x00 , 0x80 , 0x3D , 0xA1]
#FLASH_WEN()
#FLASH_PAGE_PROG(0, arr)
#time.sleep(0.1)
